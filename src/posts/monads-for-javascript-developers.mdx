...
import PostExercise, { PostExerciseInlineCode } from "../components/PostExercise"
...
This article is also a small experiment. Try clicking <PostPaneLink mdxId="monad-pane">this dashed link</PostPaneLink>. It will open a pane, as will the other dashed links. If you are at a computer (recommended), you'll find some interactive exercises as well.
...
<PostExercise codeId="with-log-monad">
Now it's your turn. Open this exercise and try implementing <PostExerciseInlineCode>runWithLog</PostExerciseInlineCode> using both <PostExerciseInlineCode>wrap</PostExerciseInlineCode> and <PostExerciseInlineCode>bind</PostExerciseInlineCode>.
...
Probably not? I personally don't think monads are that useful in JavaScript. However, ideas from functional programming are what inspired frameworks like React. Learning monads and alike gets you comfortable thinking about types at a higher level.
...
<PostExercise codeId="monad-maybe">
Try implementing <PostExerciseInlineCode>wrap</PostExerciseInlineCode> and <PostExerciseInlineCode>bind</PostExerciseInlineCode> for <PostExerciseInlineCode>Maybe</PostExerciseInlineCode>.
...
`map` applies a function to values inside a context value. The most common operation is mapping over an array. Here's `map` for `Maybe`. I'm putting it side-by-side with `bind`.

<CodeSideBySide>

```tsx
function maybeMap<A>(m: Maybe<A>, f: A => B): Maybe<B> {
  if (m === undefined) {
    return undefined;
  }
  return {
...
```tsx
function maybeBind<A>(m: Maybe<A>, f: A => Maybe<B>): Maybe<B> {
  if (m === undefined) {
    return undefined;
  }
  return f(m.value);
...
They're so similar. The heart of the comparison is the type of `f`, the function parameter. The `f` for `map` returns `B` whereas `map` for `bind` returns `Maybe<B>`.

If you pass in a `{value}` to `map`, you're definitely getting a `{value}` back (never `undefined`). However, `bind`'s given function has the power to return `undefined`. `bind`'s `f` has the ability to modify the "structure" of the context value. In a way, you can think of monad's `bind` as more powerful than `map`.
...
Another way to think about monads is with <PostPaneLink mdxId="monad-join">`join`</PostPaneLink>, a way to smash two "computational contexts" into one. For example, `WithLog<WithLog<T>> => WithLog<T>`. 

# Recap

A monad is a generic type that implement `wrap` and `bind`. They might be overhyped but they are essential in some programming languages, most notably Haskell. Monads enable side effects and allow sequencing actions in a pure functional programming language.

Please send any feedback or [follow me](https://twitter.com/_paulshen) on twitter if you want more content like this 🙏
...
- [You Could Have Invented Monads!](http://blog.sigfpe.com/2006/08/you-could-have-invented-monads-and.html) - I took a lot of inspiration (copied/translated to JavaScript?) from this great monad tutorial.
- [Comprehending Monads](https://ncatlab.org/nlab/files/WadlerMonads.pdf) - Philip Wadler's original paper on monads
- [Monads are burritos](https://chrisdone.com/posts/monads-are-burritos/) - A comic about monad tutorials (from [monad tutorial fallacy](https://chrisdone.com/posts/monads-are-burritos/))