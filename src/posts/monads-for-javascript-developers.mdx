...
I don't think monads are that useful and definitely not necessary for JavaScript developers. However, ideas from functional programming are what inspired frameworks like React. Learning monads and alike gets you comfortable thinking about types at a higher level.
...
Here's `map` for `Maybe`. I'm putting it side-by-side with `bind`.
...
They're so similar! The most notable difference is the type of `f`, the function parameter. The `f` for `map` returns `B` whereas `f` for `bind` returns `Maybe<B>`.

If you pass in a `{value}` to `map`, you're definitely getting a `{value}` back (never `undefined`). However, `bind`'s given function can return `undefined`. `bind`'s `f` has the ability to modify the "structure" of the context value. In a way, you can think of monad's `bind` as more powerful than `map`.
...
Was any part (or all of it!) confusing? Please send any comments on [twitter](https://twitter.com/_paulshen). Thanks!
